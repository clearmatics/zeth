...
contract BaseMixer is MerkleTreeMiMC7, ERC223ReceivingContract {
    ...
    function assemble_public_values(uint[] memory primary_inputs) public view returns (uint64 vpub_in, uint64 vpub_out){
    ...
    function assemble_hsig(uint[] memory primary_inputs) public view returns (bytes32 hsig){
        ...
        bytes32 hsig_bytes = (bytes32(primary_inputs[1 + 1 + 2*jsIn + jsOut]) << padding + 2*size_value) >> field_capacity;
    ...
    function assemble_nullifier(uint index, uint[] memory primary_inputs) public view returns (bytes32 sn){
        ...
        uint sn_bit_index = 2*size_value + nullifier_index * packing_residue_length;
        require(
    ...
    function assemble_commitment(uint index, uint[] memory primary_inputs) public view returns (bytes32 cm){
        ...
        // See the way the residual bits are ordered in the extended proof
        uint commitment_bit_index = 2*size_value + commitment_index * packing_residue_length;
        require(
        ...
        bytes32 cm_bytes = (bytes32(primary_inputs[1 + 1 + 2*jsIn + jsOut]) << padding + commitment_bit_index) >> field_capacity;
