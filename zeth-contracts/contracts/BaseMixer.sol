...
contract BaseMixer is MerkleTreeMiMC7, ERC223ReceivingContract {
    ...
    //TODO: refactor: merge the functions?
    //TODO: what about change the signature of assemble_root_and_nulligie by taking input bytes32[]?
    function assemble_root_and_nullifiers_and_append_to_state(uint[] memory primary_inputs) internal {
        ...
        uint256 current_primary_input;

        current_primary_input = primary_inputs[0];

        require(
            roots[bytes32(current_primary_input)],
            "Invalid root: This root doesn't exist"
        );

        // 2. We re-assemble the nullifiers (JSInputs)
        for(uint i = 1; i < 1 + jsIn; i += 1) {
            current_primary_input = primary_inputs[i];

            bytes32 current_nullifier = bytes32(current_primary_input);
            require(
    ...
    function assemble_commitments_and_append_to_state(uint[] memory primary_inputs) internal {
        ...
        for(uint i = 1 + jsIn ; i < (1 + jsIn + jsOut); i += 1) {
            current_primary_input = primary_inputs[i]; // See the way the inputs are ordered in the extended proof

            uint256 commitmentAddress = insert(bytes32(current_primary_input));
    ...
    function process_public_values(uint[] memory primary_inputs) internal {
        ...
        uint256 vpub_in = primary_inputs[1 + jsIn + jsOut];
        ...
        uint256 vpub_out = primary_inputs[1 + jsIn + jsOut + 1];
