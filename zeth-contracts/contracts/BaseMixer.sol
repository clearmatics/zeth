...
contract BaseMixer is MerkleTreeMiMC7, ERC223ReceivingContract {
    ...
    function assemble_public_values(uint[] memory primary_inputs) public pure returns (uint64 vpub_in, uint64 vpub_out){
        ...
        bytes32 vpub_bytes = bytes32(primary_inputs[1 + nb_hash_digests]) << padding_size;
        vpub_bytes = Bytes.swap_bit_order(vpub_bytes);
        ...
        vpub_bytes = bytes32(primary_inputs[1 + 1 + 2*jsIn + jsOut]) << (padding_size + public_value_length);
        vpub_bytes = Bytes.swap_bit_order(vpub_bytes);
