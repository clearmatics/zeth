...
library Bytes {
	function getDigestFromFieldElements(uint[] memory input) public pure returns (bytes1, bytes1, bytes1, bytes1, /*bytes1[] memory*/ bytes memory) {
		...
		uint8 n = 5; // We do a right shift 5 times because we only have 3 meaningful bits in inputs[1]
		// so we "eliminate" the zeroes by moving all the significant bits (significants in the sense that they correspond
		// to field data).
		...
		return (last_byte_prefix, last_byte_suffix, res, bytes1(reversed), digest);
	}

	//function testRootAssemble(uint[] memory input) public pure returns (bytes1, bytes1, bytes1, bytes1, /*bytes1[] memory*/ bytes memory) {
	//	bytes32 inverted_input1 = flip_endianness(bytes32(input[0]));
	//	//bytes32 inverted_input2 = flip_endianness(bytes32(input[1]));
	...
	//	uint8 reversed = uint8(reverseByte(aInt));
	//	uint8 shifted = reversed / 2 ** n;
	//	bytes1 shifted_byte = bytes1(shifted);     // Back to bytes. Result: 0x2d  [00101101]

	//	bytes1 res = last_byte_prefix ^ shifted_byte;

	//	//bytes1[] memory finalRoot = new bytes1[](32);
	//	bytes memory finalRoot = new bytes(32);
	//	for (uint i = 0; i < 31; i++) {
	//		finalRoot[i] = inverted_input1[i];
	//	}
	//	finalRoot[31] = res;

	//	bytes32 ultimateRes = bytesToBytes32(finalRoot, 0);

	//	return (last_byte_prefix, last_byte_suffix, res, bytes1(reversed), finalRoot);
	//}
