...
from zeth_misc_cli.eth_gen_network_config import gen_network_config
from zeth_misc_cli.eth_gen_address import gen_address
from zeth_misc_cli.eth_get_balance import get_balance
from zeth_misc_cli.eth_send import send
...
@pass_context
def eth(ctx: Context, eth_network: str) -> None:
    """
    Commands to interact with an Ethereum-like blockchain
    """
...
eth.add_command(gen_network_config)
eth.add_command(gen_address)
eth.add_command(get_balance)
eth.add_command(send)
eth.add_command(help)


if __name__ == "__main__":
    eth()  # pylint: disable=no-value-for-parameter
