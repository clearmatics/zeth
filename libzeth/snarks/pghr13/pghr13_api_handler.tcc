    ...
    zeth_proto::ExtendedProof *message)
{
    ...
    b->CopyFrom(format_hexPointBaseGroup2Affine<ppT>(proofObj.g_B.g)); // in
    G2 b_p->CopyFrom(format_hexPointBaseGroup1Affine<ppT>(proofObj.g_B.h));
    ...
    zeth_proto::VerificationKey *message)
{
    ...
    "]"; for (size_t i = 1; i < ic_length; ++i) {
        auto vk_ic_i =
            point_g1_affine_to_hex<ppT>(vk.encoded_IC_query.rest.values[i -
            1]);
        ss << ",[" << vk_ic_i << "]";
    }
    ss << "]";
