...
#ifndef __ZETH_MERKLE_PATH_SELECTOR_CIRCUITS_HPP___
#define __ZETH_MERKLE_PATH_SELECTOR_CIRCUITS_HPP___
...
// method:
//
// Left = ((1-is_right) * input) + (is_right * pathvar)
// Right = (is_right * input) + ((1 - is_right) * pathvar)
//
...
public:
    ...
    // The authentication node of the current level
    const libsnark::pb_variable<FieldT> pathvar;
    // The bit of the current level from the leaf address
    const libsnark::pb_variable<FieldT> is_right;

    // The first input of the next hash to compute
    libsnark::pb_variable<FieldT> left;
    // The seconde input of the next hash to compute
...
#endif // __ZETH_MERKLE_PATH_SELECTOR_CIRCUITS_HPP___
