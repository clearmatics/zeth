...
public:
    ...
    const libsnark::pb_variable_array<FieldT>
        address_bits; // Address of the leaf to authenticate
    const libsnark::pb_variable<FieldT> leaf; // Leaf to authenticate
    const libsnark::pb_variable_array<FieldT>
        path; // Merkle Authentication path

    std::vector<merkle_path_selector<FieldT>>
        selectors; // Gadget informing the position in the three of the computed
                   // hash and authentication node
    std::vector<HashTreeT>
        hashers; // Vector of hash gadgets to compute the intermediary hashes
