    ...
    merkle_tree_field<FieldT, HashTreeT>(depth)
{
    ...
    for (size_t address = 0; address < contents_as_vector.size(); ++address) {
        // `1ul << depth` returns the total number of leaves in the merkle tree of depth `depth`
...
void merkle_tree_field<FieldT, HashTreeT>::set_value(const size_t address, const FieldT &value) {
    ...
    hashes[idx] = value; // The last element on the merkle path is the leaf value itself
...
template<typename FieldT, typename HashTreeT>
std::vector<FieldT> merkle_tree_field<FieldT, HashTreeT>::get_path(const size_t address) const {
...
void merkle_tree_field<FieldT, HashTreeT>::dump() const {
    // `1ul << depth`  returns the total number of leaves in the merkle tree of depth `depth`
    // Print the tree leaves (stored in the `values` map)
    ...
    // and the value of the values in `hash_defaults` for a debugging/information purpose
    // To remove if this is useless.
    //
