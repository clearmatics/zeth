...
zeth_note parse_zeth_note(const prover_proto::ZethNote &note)
{
    ...
    bits384 note_trap_r =
        get_bits384_from_vector(hexadecimal_str_to_binary_vector(note.trap_r()));
    ...
    libff::alt_bn128_G1 point)
{
    ...
    std::string x_coord = "0x" + hex_string_from_libsnark_bigint(aff.X.as_bigint());
    std::string y_coord = "0x" + hex_string_from_libsnark_bigint(aff.Y.as_bigint());
