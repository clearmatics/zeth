...
template<typename FieldT>
void run_setup_and_generate_proof(protoboard<FieldT> pb, std::string path) {
    std::stringstream ss;
    std::ofstream fh;
    fh.open(path, std::ios::binary);

    // Generate verification key and proving key (Trusted setup) from the defined R1CS
    ...
    ss << "primary_input" << primary_input;
    ss << "auxiliary_input" << auxiliary_input;
