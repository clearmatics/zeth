...
from coordinator.server_state import \
    CONTRIBUTION_INTERVAL, Contributor, ServerState, \
    _server_state_to_json, _server_state_from_json
...
class TestServerState(TestCase):
    ...
    def _test_json_serialization(self, state: ServerState) -> None:
        state_json_1 = _server_state_to_json(state)
        state_2 = _server_state_from_json(state_json_1)
        state_json_2 = _server_state_to_json(state_2)
        self.assertEqual(state_json_1, state_json_2)
