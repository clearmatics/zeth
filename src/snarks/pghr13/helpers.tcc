...
void export_verification_key(libsnark::r1cs_ppzksnark_keypair<ppT> keypair)
{
    ...
    std::cout << "\tVerification key in Solidity compliant format:{"
              ...
              << point_g2_affine_as_hex(keypair.vk.alphaA_g2) << ");"
              << "\n"
              << "\t\tvk.B = Pairing.G1Point("
              << point_g1_affine_as_hex(keypair.vk.alphaB_g1) << ");"
              << "\n"
              << "\t\tvk.C = Pairing.G2Point("
              << point_g2_affine_as_hex(keypair.vk.alphaC_g2) << ");"
              << "\n"
              << "\t\tvk.gamma = Pairing.G2Point("
              << point_g2_affine_as_hex(keypair.vk.gamma_g2) << ");"
              << "\n"
              << "\t\tvk.gammaBeta1 = Pairing.G1Point("
              << point_g1_affine_as_hex(keypair.vk.gamma_beta_g1) << ");"
              << "\n"
              << "\t\tvk.gammaBeta2 = Pairing.G2Point("
              << point_g2_affine_as_hex(keypair.vk.gamma_beta_g2) << ");"
              << "\n"
              << "\t\tvk.Z = Pairing.G2Point("
              << point_g2_affine_as_hex(keypair.vk.rC_Z_g2) << ");"
              << "\n"
              << "\t\tvk.IC = new Pairing.G1Point[](" << ic_length << ");"
              << "\t\tvk.IC[0] = Pairing.G1Point("
              << point_g1_affine_as_hex(keypair.vk.encoded_IC_query.first)
              << ");" << std::endl;
    for (size_t i = 1; i < ic_length; ++i) {
        auto vk_ic_i = point_g1_affine_as_hex(
...
void display_proof(libsnark::r1cs_ppzksnark_proof<ppT> proof)
{
    std::cout << "Proof:"
              ...
              << point_g1_affine_as_hex(proof.g_A.g) << ");"
              << "\n"
              << "proof.A_p = Pairing.G1Point("
              << point_g1_affine_as_hex(proof.g_A.h) << ");"
              << "\n"
              << "proof.B = Pairing.G2Point("
              << point_g2_affine_as_hex(proof.g_B.g) << ");"
              << "\n"
              << "proof.B_p = Pairing.G1Point("
              << point_g1_affine_as_hex(proof.g_B.h) << ");"
              << "\n"
              << "proof.C = Pairing.G1Point("
              << point_g1_affine_as_hex(proof.g_C.g) << ");"
              << "\n"
              << "proof.C_p = Pairing.G1Point("
              << point_g1_affine_as_hex(proof.g_C.h) << ");"
              << "\n"
              << "proof.H = Pairing.G1Point("
              << point_g1_affine_as_hex(proof.g_H) << ");"
              << "\n"
              << "proof.K = Pairing.G1Point("
              << point_g1_affine_as_hex(proof.g_K) << ");" << std::endl;
};
    ...
    boost::filesystem::path path)
{
    ...
    ss << "\"IC\" :[[" << point_g1_affine_as_hex(vk.encoded_IC_query.first)
       << "]";
