...
void export_verification_key(libsnark::r1cs_gg_ppzksnark_keypair<ppT> keypair)
{
    ...
    for (size_t i = 1; i < abc_length; ++i) {
        auto vk_abc_i =
            get_point_g1_affine_as_hex_str(keypair.vk.ABC_g1.rest.values[i - 1]);
        std::cout << "\t\tvk.ABC[" << i << "] = Pairing.G1Point("
                << vk_abc_i
                << ");" << "\n";
    }
    ...
    boost::filesystem::path path)
{
    if (path.empty()) {
        // Used for a debugging purpose
        boost::filesystem::path tmp_path = get_path_to_debug_directory();
        boost::filesystem::path proof_and_inputs_json_file("proof_and_inputs.json");
